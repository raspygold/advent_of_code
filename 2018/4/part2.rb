...
def solve(input)
  ...
  guards_most_common_sleeps = guards.map do |gid, shifts|
    freq = (0..59).inject({}) do |hsh, min|
      hsh[min] = shifts.count {|sleeps| sleeps.include?(min)}
      hsh
    end
    freq.max_by {|x| x[1]}.reverse << gid
  end
  
  guards_most_common_sleeps.sort!.reverse!
  guards_most_common_sleeps.first[1..-1].reduce(&:*)
end
## SOLUTION ENDS
