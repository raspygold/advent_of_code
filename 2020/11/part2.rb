...
class Day11P1 < Puzzle
  ...
  def adjacent_seats(map, y, x)
    possible_mods = ([-1,0,1].product([-1,0,1]) - [[0,0]])

    possible_mods.map do |mod|
      orig_mod = mod.dup
      cell = nil
      loop do
        break unless (y + mod[0]).between?(0, @map_dimensions[0]-1) && (x + mod[1]).between?(0, @map_dimensions[1]-1)
        cell = map[y + mod[0]][x + mod[1]]
        break if cell && cell != FLOOR
        mod = mod.map.with_index { |c, i| c += orig_mod[i] }
      end
